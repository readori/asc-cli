...
struct GalleryApplyTests {
    ...
    @Test func `side by side template creates fewer screens from same screenshots`() {
        ...
        let singleDevice = ScreenTemplate(
            headline: TextSlot(y: 0.04, size: 0.08),
            device: DeviceSlot(y: 0.18, width: 0.85)
        )
        let singleScreens = Gallery.distributeScreenshots(screenshots, screenTemplate: singleDevice)
        ...
        let dualScreens = Gallery.distributeScreenshots(screenshots, screenTemplate: dualDevice)
        ...
        let tripleDevice = ScreenTemplate(
            headline: TextSlot(y: 0.04, size: 0.08),
            devices: [
        ...
        let tripleScreens = Gallery.distributeScreenshots(screenshots, screenTemplate: tripleDevice)
