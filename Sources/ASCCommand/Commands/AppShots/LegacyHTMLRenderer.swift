...
enum LegacyHTMLRenderer {
    ...
    static func render(
        ...
        // Legacy uses a single mockup for all screens (keyed as "__default__")
        let mockupInfo = assets.mockups["__default__"]

        let screenshotCards = plan.screens.map { screen in
            let dataURI = matchScreenshot(screen: screen, dataURIs: assets.screenshotDataURIs)
